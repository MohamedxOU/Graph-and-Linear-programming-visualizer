...
from algorithms.lp_algos.simplex_standard import standard_simplex

class StandardSimplexPage(QWidget):
    ...
    def init_ui(self):
        ...
        # Title
        title = QLabel("Standard Simplex Method (≤ constraints only)")
        ...
        # Run button
        self.btn_run = QPushButton("Run Standard Simplex Algorithm")
    ...
    def run_simplex(self):
        ...
        try:
            self.result_area.setPlainText("Running Standard Simplex Algorithm...\n")
            result = standard_simplex(c, A, b, maximize)
            output = []
            output.append("=== Standard Simplex Method Results ===")
            ...
            if result['status'] == 'optimal' and result['solution'] is not None:
                ...
                output.append("\nProblem cannot be solved with the standard simplex method.")
            else:
                output.append(f"\n{result['status']}")
            self.result_area.setPlainText("\n".join(output))
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to run standard simplex:\n\n{str(e)}")
